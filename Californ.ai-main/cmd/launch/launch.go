...
Examples:
  ...
  clfnai launch claude --model <model>
  clfnai launch droid --config (does not auto-launch)
  clfnai launch codex -- -p myprofile (pass extra args to integration)
  clfnai launch codex -- --sandbox workspace-write`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if name == "" {
				if cmd.Flags().Changed("model") || cmd.Flags().Changed("config") || cmd.Flags().Changed("yes") || len(passArgs) > 0 {
					return fmt.Errorf("flags and extra args require an integration name, for example: 'clfnai launch claude --model qwen3.5'")
				}
				runTUI(cmd)
				return nil
			}
...
func (c *launcherClient) selectSingleModelWithSelector(ctx context.Context, title, current string, selector SingleSelector) (string, error) {
	...
	items, _, err := c.loadSelectableModels(ctx, nil, current, "no models available, run 'clfnai pull <model>' first")
	if err != nil {
		return "", err
	}
