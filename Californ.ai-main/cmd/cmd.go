...
func CreateHandler(cmd *cobra.Command, args []string) error {
	...
	if err := client.Create(cmd.Context(), req, fn); err != nil {
		if strings.Contains(err.Error(), "path or Modelfile are required") {
			return fmt.Errorf("the californ.ai server must be updated to use `clfnai create` with this client")
		}
		return err
	}
...
func WebHandler(cmd *cobra.Command, args []string) error {
	url := "https://california.framer.ai"
	fmt.Printf("Opening California.ai web interface: %s\n", url)
...
var (
	antiramModels = make(map[string]bool)
	antiramOnce sync.Once
)

func registerAntiramModel(modelName string) error {
	antiramModels[modelName] = true
	
	// Setup shutdown handler only once
	antiramOnce.Do(func() {
		setupAntiramCleanup()
	})
...
func setupAntiramCleanup() {
	c := make(chan os.Signal, 1)
	signal.Notify(c, os.Interrupt, syscall.SIGTERM)
	
	go func() {
		<-c
		cleanupAntiramModels()
		os.Exit(0)
	}()
}

func cleanupAntiramModels() {
	if len(antiramModels) == 0 {
		return
	}
	
	fmt.Println("\nCleaning up antiram models...")
	client, err := api.ClientFromEnvironment()
	if err != nil {
		fmt.Printf("Failed to create client for cleanup: %v\n", err)
		return
	}
	
	for modelName := range antiramModels {
		fmt.Printf("Deleting antiram model: %s\n", modelName)
		
		// Stop the model first if it's running
		stopReq := &api.GenerateRequest{
			Model:     modelName,
			KeepAlive: &api.Duration{Duration: 0},
		}
		if err := client.Generate(context.Background(), stopReq, func(r api.GenerateResponse) error { return nil }); err != nil {
			fmt.Printf("Warning: failed to stop model %s: %v\n", modelName, err)
		}
		
		// Delete the model using the correct API
		if err := client.Delete(context.Background(), &api.DeleteRequest{Name: modelName}); err != nil {
			fmt.Printf("Failed to delete model %s: %v\n", modelName, err)
		} else {
			fmt.Printf("Successfully deleted antiram model: %s\n", modelName)
		}
	}
}
...
func NewzipHandler(cmd *cobra.Command, args []string) error {
	if len(args) != 2 {
		return fmt.Errorf("usage: clfnai newzip <model_name> <zip_file>")
	}
	...
	if _, err := os.Stat(zipFilePath); os.IsNotExist(err) {
		return fmt.Errorf("zip file not found: %s", zipFilePath)
	}

	// Create temporary directory for extraction
	tempDir, err := os.MkdirTemp("", "clfnai-model-*")
	if err != nil {
	...
	// Extract zip file
	fmt.Printf("Extracting %s to temporary directory...\n", zipFilePath)
	if err := extractZip(zipFilePath, tempDir); err != nil {
		return fmt.Errorf("failed to extract zip file: %w", err)
	}

	// Create Modelfile in the extracted directory
	modelfilePath := filepath.Join(tempDir, "Modelfile")
	modelfileContent := fmt.Sprintf("FROM .\n")
	if err := os.WriteFile(modelfilePath, []byte(modelfileContent), 0644); err != nil {
		return fmt.Errorf("failed to create Modelfile: %w", err)
	}

	// Change to the extracted directory for model creation
	oldDir, _ := os.Getwd()
	defer os.Chdir(oldDir)
	if err := os.Chdir(tempDir); err != nil {
		return fmt.Errorf("failed to change to model directory: %w", err)
	}

	// Create the model using existing create functionality
	fmt.Printf("Creating model '%s' from extracted files...\n", modelName)
	
	// Reuse existing CreateHandler logic but with our extracted files
	client, err := api.ClientFromEnvironment()
	if err != nil {
		return err
	}
	...
	// Parse the Modelfile we created
	modelfile, err := parser.ParseFile(strings.NewReader(modelfileContent))
	if err != nil {
		return fmt.Errorf("failed to parse Modelfile: %w", err)
	}

	req, err := modelfile.CreateRequest(tempDir)
	if err != nil {
		return err
	}
	req.Model = modelName
	...
	fn := func(resp api.ProgressResponse) error {
		if resp.Digest != "" {
			...
			if !ok {
				msg := resp.Status
				if msg == "" {
					msg = fmt.Sprintf("processing %s...", resp.Digest[7:19])
				}
				bar = progress.NewBar(msg, resp.Total, resp.Completed)
				bars[resp.Digest] = bar
				p.Add(resp.Digest, bar)
	...
	fmt.Printf("✅ Successfully created model '%s' from zip file!\n", modelName)
	fmt.Printf("You can now run it with: clfnai run %s\n", modelName)

	return nil
}

func extractZip(zipPath, destDir string) error {
	// This is a simple zip extraction implementation
	// In a real implementation, you'd use archive/zip package
	cmd := exec.Command("unzip", "-q", zipPath, "-d", destDir)
	if err := cmd.Run(); err != nil {
		// Fallback to Go implementation if unzip not available
		return extractZipGo(zipPath, destDir)
	}
	return nil
}

func extractZipGo(zipPath, destDir string) error {
	// Basic zip extraction using Go's archive/zip package
...
func RunHandler(cmd *cobra.Command, args []string) error {
	...
	// Check for antiram suffix
	isAntiram := strings.HasSuffix(name, "antiram")
	var originalModelName string
	if isAntiram {
		originalModelName = strings.TrimSuffix(name, "antiram")
		name = originalModelName
		opts.Model = name
		fmt.Printf("Running model '%s' in anti-RAM mode (will auto-delete on shutdown)\n", name)
		
		// Register model for auto-deletion on shutdown
		if err := registerAntiramModel(originalModelName); err != nil {
			fmt.Printf("Warning: failed to register antiram model for auto-deletion: %v\n", err)
		}
	}
	...
	// For antiram models, set KeepAlive to 0 to unload immediately when done
	if isAntiram && opts.KeepAlive == nil {
		opts.KeepAlive = &api.Duration{Duration: 0}
	}
	...
	if isEmbeddingModel {
		if opts.Prompt == "" {
			return errors.New("embedding models require input text. Usage: clfnai run " + name + " \"your text here\"")
		}
	...
	if slices.Contains(info.Capabilities, model.CapabilityImage) {
		if opts.Prompt == "" && !interactive {
			return errors.New("image generation models require a prompt. Usage: clfnai run " + name + " \"your prompt here\"")
		}
...
func ListHandler(cmd *cobra.Command, args []string) error {
	...
	for _, m := range models.Models {
		// If no args or args[0] is "models", list all models
		...
		} else if strings.HasPrefix(strings.ToLower(m.Name), strings.ToLower(args[0])) {
			// Legacy behavior: filter by model name prefix
			var size string
			if m.RemoteModel != "" {
				size = "-"
			} else {
...
func listCommands(cmd *cobra.Command) error {
	commands := []struct {
		...
		{"rm", "Remove a model"},
		{"cp", "Copy a model"},
		{"serve", "Start Californ.ai server"},
		{"launch", "Launch integrations (claude, codex, etc.)"},
		{"newzip", "Add an AI model from a zip file"},
		{"web", "Open California.ai web interface"},
		{"signin", "Sign in to californ.ai"},
		{"signout", "Sign out from californ.ai"},
	}

	var data [][]string
	for _, cmd := range commands {
		data = append(data, []string{cmd.name, cmd.description})
	}
	...
	fmt.Println("  clfnai list                    # List models")
	fmt.Println("  clfnai list models             # List models")
	fmt.Println("  clfnai list commands           # List commands")
	fmt.Println("  clfnai list <model_prefix>     # Filter models by prefix")
...
// ensureServerRunning checks if the californ.ai server is running and starts it in the background if not.
...
func NewCLI() *cobra.Command {
	...
	listCmd := &cobra.Command{
		Use:     "list [models|commands]",
		Aliases: []string{"ls"},
		Short:   "List models or commands",
		Args:    cobra.MaximumNArgs(1),
		PreRunE: func(cmd *cobra.Command, args []string) error {
			// Only check server heartbeat if we're listing models
			if len(args) == 0 || args[0] == "models" {
				return checkServerHeartbeat(cmd, args)
			}
			return nil
	...
	newzipCmd := &cobra.Command{
		Use:     "newzip MODEL_NAME ZIP_FILE",
		Short:   "Add an AI model from a zip file to Californ.ai",
		Args:    cobra.ExactArgs(2),
		PreRunE: checkServerHeartbeat,
		RunE:    NewzipHandler,
	}

	newzipCmd.Flags().StringP("quantize", "q", "", "Quantize model to this level (e.g. q4_K_M)")

	webCmd := &cobra.Command{
		Use:     "web",
		Short:   "Open California.ai web interface",
		Args:    cobra.ExactArgs(0),
		RunE:    WebHandler,
	}
